...
const CoursePlayerPage = () => {
    ...
    useEffect(() => {
        const fetchProgress = async () => {
            if (user && currentLecture && currentLecture.type !== 'quiz') {
                const data = await videoProgressService.getProgress(currentLecture.id, user.id);
                if (data && data.watchedPosition) {
                    setVideoProgress(data.watchedPosition);
                } else {
                    setVideoProgress(0);
                }
                setLastSavedTime(0); // Reset local tracker so next event correctly triggers a save
            }
        };
        fetchProgress();
    }, [user, currentLecture]);
        ...
        <div className="course-player-page">
            ...
            <div className="player-body">
                                                ...
                                                <video
                                                    ...
                                                    onTimeUpdate={(e) => {
                                                        ...
                                                        if (currentTime - lastSavedTime >= 5 || currentTime < lastSavedTime - 5) {
                                                            videoProgressService.saveProgress(currentLecture.id, user.id, currentTime);
                ...
                <div className={`player-sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
                    ...
                    <div className="sidebar-co ntent">
                        {activeTab === 'lectures' && (
